...
$D rm -f ledgerdb orderdb dispatchredis admindb >/dev/null 2>&1 || true
...
trap "$D rm -f ledgerdb orderdb dispatchredis admindb >/dev/null 2>&1 || true" EXIT
...
fail=0
for f in apps/svc-payment/test/ledger-service.spec.ts apps/svc-order/test/outbox-timers.spec.ts apps/svc-dispatch/test/dispatch-redis.spec.ts apps/e2e/test/order-flow.e2e.spec.ts apps/svc-payment/test/cod.spec.ts apps/svc-admin/test/audit.spec.ts; do
  echo "── $f"
  out=$(npx tsx "$f" 2>&1)
