...
$D rm -f ledgerdb orderdb >/dev/null 2>&1 || true
...
trap "$D rm -f ledgerdb orderdb >/dev/null 2>&1 || true" EXIT
...
fail=0
for f in apps/svc-payment/test/ledger-service.spec.ts apps/svc-order/test/outbox-timers.spec.ts; do
  echo "── $f"
  out=$(npx tsx "$f" 2>&1)
